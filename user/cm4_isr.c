...
extern uint16 counter_now ;
extern uint16 counter_last;
...
void pit0_ch0_isr()                     // 定时器通道 0 周期中断服务函数      
{
    ...
    tranced_gyro_z = imu660ra_gyro_transition(imu660ra_gyro_z) / 200;

    

    if (imu.broken_road_flag == 1)
    {        
        gyro_summation();     
    }
    else
    ...
    if(!PID.stop_flag&&memory_f)      store_error();
    if(!PID.stop_flag&&speedup_flag)
    {
        read_error();
        speed_judge();
    }    
    ...
    //四元数解算 惯导 
    if(nav_record_flag || N.Nag_SystemRun_Index == 2)
    {
